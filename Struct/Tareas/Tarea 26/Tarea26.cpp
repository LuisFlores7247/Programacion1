...
void leerArchivoBinario(jugadores equipo[TAM]);
void jugadoresEquipo(char equipo[30]);
...
void llenar(jugadores equipo[TAM])
{
	...
	int n;
	cout << endl
		 << "Llenando con datos aleatorios el arreglo de jugadores ....\n";
	for (int i = 0; i < TAM; i++)
	{
		...
		// generar un numero aleatorio para asignarle una edad al jugador, las edades son entre 17 y 34 a�os
...
void imprimir(jugadores equipo[TAM])
{
	...
	cout << endl
		 << "Imprimiendo el arreglo de jugadores ....\n";
...
void generarArchivoBinario(jugadores equipo[TAM])
{
	cout << "\nGenerando Archivo Binario .... " << endl;
	fstream file;
	file.open("jugadores.dat", ios::binary | ios::out | ios::app);
	if (!file)
	{
	...
	for (int i = 0; i < TAM; i++)
	{
		file.write((char *)(&equipo[i]), sizeof(jugadores));
	}
...
void leerArchivoBinario(jugadores equipo[TAM])
{
	cout << "Leyendo la informacion del Archivo Binario ....\n";
	fstream file;
	file.open("jugadores.dat", ios::binary | ios::in);
	...
	for (int i = 0; i < TAM; i++)
	{
		if (file.read((char *)(&equipo[i]), sizeof(jugadores)))
		{
			cout << endl; // salto de linea
			cout << left << setw(3) << i + 1;
			cout << left << setw(40) << equipo[i].nomjug;
...
void jugadoresEquipo(char equipo[30])
{
	cout << "Imprimiendo jugadores del equipo: " << equipo << endl;
	...
	while (file.read((char *)(&jugador), sizeof(jugadores)))
	{
		if (strcmp(jugador.eq, equipo) == 0)
		{
			encontrado = true;
			cout << "Nombre: " << jugador.nomjug << endl;
			cout << "Equipo: " << jugador.eq << endl;
			cout << "Edad: " << jugador.edad << endl;
			cout << "Sueldo: " << jugador.sueldo << endl;
			cout << endl;
		}
	}
	...
	if (!encontrado)
	{
		cout << "Equipo no registrado" << endl;
	}
}
